...
type Capture = Record<string, unknown> & { operation: string };

function requiredString(input: Capture, key: string, max = 2000): string {
  const value = input[key];
  if (typeof value !== 'string' || !value.trim() || value.length > max) throw new Error(`${key} is required`);
  return value.trim();
}

function boundedNumber(value: unknown, fallback: number, minimum = 0, maximum = 100): number {
...
export async function captureResearch(database: Database, input: Capture): Promise<Record<string, unknown>> {
  switch (input.operation) {
    ...
    case 'prediction': {
      ...
      const key = typeof input.key === 'string' && input.key ? input.key : slugify(`${thesisId}-${statement}`).slice(0, 90);
    ...
    case 'insight': {
      ...
      const insightSlug = typeof input.slug === 'string' && input.slug ? input.slug : slugify(title);
      const type = typeof input.insight_type === 'string' ? input.insight_type : 'derived';
      ...
      const nodes = Array.isArray(input.nodes) ? input.nodes.filter((value): value is string => typeof value === 'string').slice(0, 50) : [];
    ...
    case 'cycle': {
      ...
      const key = typeof input.key === 'string' && input.key ? input.key : slugify(`cycle-${thesisId}-${hypothesis}`).slice(0, 90);
