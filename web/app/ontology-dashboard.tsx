...
import { isJsonObject, isJsonString, parseJson } from '../shared/json';
...
function describeSchedule(rrule:string){
  ...
  return [days||clean(parts.get('FREQ')||'scheduled'),hours].filter(Boolean).join(' · ');
}
...
function AutomationsSurface({data}:{data:Snapshot}){
  ...
  return <section className="automation-ledger">
    <div className="automation-hero"><div><span>CLOUDFLARE / SCHEDULED OPERATIONS</span><h1>Worker observability</h1><p>Every scheduled job and execution, with runtime health plus the evidence, decisions, and durable learning it left behind.</p></div><div className="automation-kpis"><span><b>{automations.length}</b> JOBS</span><span className="green"><b>{passed}</b> PASS</span><span className="red"><b>{failed}</b> FAIL</span><span><b>{formatDuration(average)}</b> AVG</span></div></div>
