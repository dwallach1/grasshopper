...
Tabs are spelled-out words: **Board, Book, Theses, Events, Tests, Team**. **Board** is the landing page at `/` — parchment Liveline of each steward’s % vs that book’s own start, then % standings in native units (`/leaderboard` redirects here). **Book** is `/book`. The chrome stays mounted; tab switches paint from the in-memory ledger payload (`history.pushState`, no refetch). Keyboard: `1-6` panels, `g` then letter (`p/b/t/c/e/m`), `j/k` thesis, `r` refresh, `?` help. There is no filter box. Last QUANTANAMO run is a header chip, not a tab. See `docs/liveline-pnl.md`.
...
On **Board** (`/`) a quiet parchment Liveline draws each live steward’s return curve on a shared % axis (lerp between ledger prints; idle when every plotted book is stale). Standings still rank each steward’s % vs that book’s own start. On **Book** (`/book`) lots stay a quiet table. Not `dashboard_snapshots.current`, and not `position_episodes`. Missing marks stay **not in ledger**. A refresh (or the 15s poll / realtime) picks up a new QUANTANAMO snapshot without a restart. The fill log is `broker_fills` when present, otherwise filled `trade_intents`, newest first. Empty fills say **not in ledger**. Sparse snapshots are honest — the desk does not invent prints.
...
On **Theses** both desks use the parchment list (name, stance/status, confidence, domain/steward). A quiet mast strip counts live learning health from `/api/desk` (To-review, lessons open vs in playbook, beliefs in force, tagged lots vs legacy untagged vs missing-gate open books). **Beliefs** lists the newest playbook rule per thesis (rationale, thesis id+title, which live lot binds it). **Tagged lots** lists open books that carry a `thesis_id`. Lesson summaries already on **Lessons** are not repeated on a belief card. Tap a thesis for the sentence, falsifier, belief trail, and evidence. No Book/Board marks on the card. **Lessons** lists `research_lessons` (open first, then in playbook). A **To review** queue lists pending `ontology_candidates` (membership first, including `source_count=1`; ledger score, cap 40; deny-list junk is omitted). The public phone is read-only. The local operator desk can incorporate a lesson (writes a `playbook_rule` belief) and promote / reject / merge candidates. See [`docs/lesson-incorporate.md`](docs/lesson-incorporate.md) and [`docs/ontology-review.md`](docs/ontology-review.md).
...
The desk stays read-only for thesis status, evidence, and trading. QUANTANAMO writes those. Operator writes are ontology candidate review (`POST /api/ontology/review`) and lesson incorporate (`POST /api/lessons/incorporate`), gated by `ledger_operators`. Public phone has no write chrome.
...
- `POST /api/lessons/incorporate` is the operator lesson → playbook path (not on the public build).
...
7. Thesis status and evidence stay ledger reads. Candidate promote/reject/merge and lesson incorporate are operator-only.
