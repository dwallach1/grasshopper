...
export const FinancialRequestSchema = z.object({
  endpoint: z.string().trim().min(1).regex(/^[a-z0-9/_-]+$/i, {
    error: 'valid endpoint is required',
  }),
...
async function fingerprint(spec: FinancialRequest): Promise<string> {
  return sha256(canonicalJson({
    ...
    params: spec.params || {},
    body: (spec.body || null) as JsonValue,
  }));
}
...
function records(payload: JsonValue): JsonObject[] {
  ...
  const arrays = Object.values(payload).filter(
    (item): item is JsonObject[] => Array.isArray(item) && item.every(isJsonObject),
  );
...
async function normalize(
  ...
  const rows = await Promise.all(records(payload).map(async (record, index) => {
    ...
    const recordKey = await sha256(
      canonicalJson(Object.keys(identity).length ? identity : { index, query: await fingerprint(spec) }),
    );
    ...
    return {
      ...
      period: record.period || spec.params?.period || null,
      report_period: record.report_period || record.report_date || record.date || null,
      filing_date: record.filing_date || record.accepted_date || null,
      fetched_at: fetchedAt,
      record_sha256: await sha256(payloadJson),
      payload_json: record,
...
async function decodeStored(body: Uint8Array, encoding: string): Promise<JsonValue> {
  ...
  return asJsonValue(JSON.parse(new TextDecoder().decode(bytes)) as unknown);
}
...
export async function fetchFinancialData(
  ...
  const spec = FinancialRequestSchema.parse(rawSpec);
  ...
  if (!spec.force) {
    ...
    if (cached[0]) {
      await database.execute(
        "insert into financial_access_log(request_fingerprint,request_id,access_type,accessed_at,detail) values ($1,$2,'cache',now(),'fresh worker response')",
        [requestFingerprint, cached[0].id],
      );
      return {
        payload: await decodeStored(cached[0].response_body, cached[0].response_encoding),
        source: 'cache',
        requestId: Number(cached[0].id),
        normalized: 0,
        status: Number(cached[0].status_code),
      };
    }
  }
  ...
  for (const [key, value] of Object.entries(spec.params || {}).sort()) {
  ...
  try {
    payload = asJsonValue(JSON.parse(text) as unknown);
  } catch {
  ...
  `, [
    ...
    spec.body ? JSON.stringify(spec.body) : null,
    startedAt,
    completedAt,
    response.status,
  ...
  await database.execute(
    "insert into financial_access_log(request_fingerprint,request_id,access_type,accessed_at,detail) values ($1,$2,'network',now(),$3)",
    [requestFingerprint, requestId, `HTTP ${response.status}`],
  );
