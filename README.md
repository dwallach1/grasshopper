...
| 3 | Theses | Phone: parchment list of ledger theses (name, stance/status, confidence, domain/steward). Mast learning pulse (To-review, lessons open vs in playbook, beliefs in force, thesis vs legacy untagged vs missing-gate open books) from `/api/desk` — no invented scores. Tap opens summary, falsifier, belief/confidence trail, and related evidence — no Book/Board marks. Operator desk keeps the ledger table (lifecycle, evidence, held/candidate symbols, lessons). |
