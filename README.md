...
Chrome stays mounted. Tab switches paint from the in-memory ledger payload. On the phone, Board / Book / Team are one swipe deck; the dock labels are a thin indicator, not a second nav. `prefers-reduced-motion` snaps without the travel animation. Keyboard: `1–6`, `g` then letter (`p/b/t/c/e/m`), `j/k` thesis, `r` refresh, `?` help. Venue chips (All / STOCKS / PREDICTIONS / COINS) filter Book, Theses, and Events — not a search box.
