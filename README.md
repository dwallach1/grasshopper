...
| Exits | The lot's own invalidation, then the linked thesis's | `position_episodes.invalidation_price` (exit when the price is at or below it) / `invalidation_note`, then `theses.falsifier`; with none written, the steward's judgment and learned beliefs |
...
| 3 | Theses | Phone: parchment list of ledger theses (name, stance/status, confidence, domain/steward). Mast learning pulse (To-review, lessons open vs in playbook, beliefs in force, tagged vs legacy untagged vs missing-gate open lots) from `/api/desk` — no invented scores. Inspectable **Beliefs** and **Tagged lots** parchment show the playbook rationale / thesis id+title / which live lot binds them. Tap a thesis for summary, falsifier, belief/confidence trail, and related evidence — no Book/Board marks. Operator desk keeps the ledger table (lifecycle, evidence, held/candidate symbols, lessons). |
...
| 6 | Team | One scrollable stack of parchment steward cards from `desk_agents` (QUANTANAMO, ODDSBORNE, BANDIT, plus COINTANAMO if present). Circle+eyes face, name, domain, and pulse (heartbeat age). No carousel, win rate, hold, size, or NAV. Empty `desk_agents` is an honest empty. `/mates` redirects here. |
...
Chrome stays mounted. Tab switches paint from the in-memory ledger payload. On the phone, Board / Book / Theses / Team are one circular swipe deck; the dock labels are a thin indicator, not a second nav. Pull down at the top of a pane to re-fetch `/api/desk` (or `/api/ledger` on the operator desk). Theses (`/theses`) is a parchment card list on that same rail — tap for the sentence and evidence. Keyboard: `1–6`, `g` then letter (`p/b/t/c/e/m`), `j/k` thesis, `r` refresh, `?` help. Venue chips (All / STOCKS / PREDICTIONS / COINS) filter Book, Theses, and Events — not a search box.
...
| Domain | Tables |
|---|---|
| Research | `theses`, `thesis_evidence`, `thesis_scores`, `catalysts`, `research_queue`, `research_lessons`, `belief_updates` (playbook rules; view `active_playbook_rules`) |
