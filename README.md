...
| Key | Tab | Shows |
|---|---|---|
| 1 | Book | Landing (`/`). One book, three venues: QUANTANAMO stocks (Agentic last4 7638 NAV / cash / lots from `portfolio_exposure` + `account_snapshots`), ODDSBORNE predictions (`pm_positions` / `pm_pnl` when present), and BANDIT coins (`meme_positions` / `meme_pnl` when present). **All** rolls up USD books (Stocks NAV/cash + Predictions equity/cash) and keeps SOL as a separate native line — never converted, never shown as `$`. Lots / fills / intents are the union. STOCKS / PREDICTIONS / COINS chips keep their own units (COINS is SOL). Living diagnostic is the Agentic 7638 path (STOCKS chip only). Unmarked lots are muted, never a fake P/L color. FILLS includes a tap-to-replay tape (`TradeReplay` + `DeskRive`) over the same `fill_log` — Rive is motion chrome; fills/marks stay ledger-only. |
