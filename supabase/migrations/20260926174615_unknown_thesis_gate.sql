-- An unknown thesis id (no row in public.theses) never sizes an entry, for any steward:
-- entry_allowed = false, sized_notional = 0, entry_blocked_reason = 'unknown_thesis',
-- multiplier 0. A null thesis is unchanged: QUANTANAMO still requires one
-- ('quantanamo_requires_thesis'); ODDSBORNE and BANDIT size untagged entries at the
-- steward-level multiplier. Same signature and return shape as 17.
...
begin
  ...
  if p_thesis_id is not null then
    ...
    -- entries at the steward-level multiplier.
  ...
  if v_gate_applies then
    ...
    v_allowed := coalesce(v_gate, false) and not v_blocked;
    v_reason := case
      when v_unknown then 'unknown_thesis'
  ...
  return query select
    ...
    case when v_gate_applies
      then 'size = requested x multiplier, limited only by spendable cash (no margin); autonomous equity entries need a hardening thesis at confidence >= 80; an unknown, rejected or killed thesis blocks new entries; no per-position cap'
      else 'size = requested x multiplier, limited only by spendable cash (no margin); the multiplier is the throttle (no confidence gate); an unknown, rejected or killed thesis blocks new entries; no per-position cap'
    end::text;
end;
$$;
