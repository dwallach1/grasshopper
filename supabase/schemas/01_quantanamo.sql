...
-- Immutable originals live in private object storage (legacy Supabase Storage or Cloudflare R2).
-- Postgres owns the queryable metadata, extracted text, and evolving judgments.
...
-- Sentiment belongs to an entity in a document, never to a Storage folder.
-- Versioned provenance lets later models revise a judgment without moving files.
...
-- QUANTANAMO backtest exhaust. JSON lives in payload_json; storage_* may be null.
-- Prices come from Financial Datasets. The desk must not invent curves or marks.
...
create index backtest_artifacts_test_id_idx on public.backtest_artifacts(test_id, created_at desc);
create index backtest_artifacts_thesis_id_idx on public.backtest_artifacts(thesis_id);
...
-- Cloudflare Workflows and Queues write their canonical run/task audit here.
-- Durable Objects retain coordination state only and can be rebuilt from these rows.
...
create unique index idx_position_episodes_one_open on public.position_episodes(account_key, symbol)
...
create unique index idx_position_monitor_event_task on public.position_monitor_events(cloud_task_id)
  where cloud_task_id is not null;
create index idx_position_monitor_events_episode on public.position_monitor_events(position_episode_id, observed_at desc);
...
create table public.trade_intents (
  ...
  status text not null default 'draft' check (status in ('draft', 'gated', 'awaiting_confirmation', 'confirmed', 'submitting', 'submitted', 'partially_filled', 'filled', 'cancelled', 'rejected', 'blocked')),
  symbol text not null references public.symbols(symbol),
  side text not null check (side in ('buy', 'sell')),
...
create index idx_trade_intents_trade_proposal on public.trade_intents(trade_proposal_id) where trade_proposal_id is not null;
create index idx_trade_intents_position_episode on public.trade_intents(position_episode_id) where position_episode_id is not null;
create index idx_trade_intents_symbol on public.trade_intents(symbol);
create index idx_trade_intents_account_snapshot on public.trade_intents(account_snapshot_id) where account_snapshot_id is not null;
