...
const PUBLIC_QUERY: Record<string, string> = {
  ...
  candidates: 'ontology_candidates?select=id,candidate_type,candidate_key,proposed_theme_id,proposed_label,proposed_description,score,evidence_count,source_count,status,last_seen_at,review_note&status=eq.pending&source_count=gte.2&order=score.desc,source_count.desc,id.desc&limit=200',
};
...
const REQUIRED: Array<[string, string]> = [
  ...
  ['positions', 'position_episodes?select=id,account_key,symbol,status,quantity,average_cost,opened_at,closed_at,next_review_at,thesis_id,untagged:meta->>untagged&status=in.(proposed,open,closing,closed)&order=opened_at.desc.nullslast,symbol.asc&limit=400'],
  ...
  ['candidates', 'ontology_candidates?select=id,candidate_type,candidate_key,proposed_theme_id,proposed_label,proposed_description,score,evidence_count,source_count,status,last_seen_at,review_note&source_count=gte.2&order=status.asc,score.desc,source_count.desc,id.desc&limit=200'],
