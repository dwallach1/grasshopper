...
2. Update local thesis memory with `npm run thesis:ingest`.
...
4. Read `npm run thesis:report` and inspect `data/thesisforge.sqlite` as needed.
...
8. Persist new observations into `data/thesisforge.sqlite`: portfolio exposure, catalysts, softened/hardened thesis scores, and trade proposals.
