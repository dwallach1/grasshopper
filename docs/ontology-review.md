...
**Real tickers are exempt for memberships.** The labels above are *words*. A membership candidate's label is a *ticker*, so when it is an active row in `public.listed_equities` (tickers verified against a live Robinhood quote and/or Financial Datasets), neither SQL `private.ontology_label_is_junk` nor TS `isJunkOntologyLabel` rejects it; review decides on context (TXN in a git blog still means "transaction"). The desk reads the PostgREST computed field `ontology_candidates.listed_equity`. Term and theme candidates still use the word list (`it`, `uk`, `race` stay junk as words). To add a ticker, check a live quote first, then `insert into public.listed_equities(symbol, name, exchange, source, verified_price, verified_at) values (...)` (QUANTANAMO's worker role can insert). On 2026-09-26 the registry was seeded with 35 verified tickers that the word list had caught (TXN, GFS, MP, ADM, COF, IT, MS, …), and 58 membership rows auto-rejected as `junk_deny_list` were restored to `pending` (`action = restore`, actor `listed_equities_migration`). Manual rejections (MP "podcast acronym bleed") stay rejected.
