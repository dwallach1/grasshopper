...
describe('desk IA smoke', () => {
  test('chrome lands on Board; Book is /book; Home and Risk are gone as destinations', async () => {
    ...
    expect(book).not.toContain('assembleBookEdge');
    ...
    expect(book).toContain('faces={faces}');
  ...
  test('product chrome is grasshopper; venue shorts are STOCKS / PREDICTIONS / COINS', async () => {
    ...
    expect(book).not.toContain('assembleBookEdge');
    expect(book).not.toContain('asciiFillBar');
    expect(book).toContain('crt-book');
    expect(book).toContain('stewardDeskFaces');
    expect(book).not.toContain('GOOD');
    expect(book).not.toContain('HOLD');
    expect(book).not.toContain('StewardEdgeCard');
    ...
    expect(team).toContain('assembleTeamRoster');
    expect(team).toContain('team-roster');
    expect(team).toContain('data-desk-nested-scroll');
    expect(team).toContain('team-card');
    expect(team).toContain('StewardAvatar');
    expect(team).toContain('size="board"');
    expect(team).not.toContain('StewardIdCard');
    expect(team).not.toContain('steward-deck');
    expect(team).not.toContain('steward-dragger');
    expect(team).not.toContain('team-card-handle');
    expect(team).not.toContain('data-team-card');
    expect(team).not.toContain('data-card-dragger');
    expect(team).not.toContain('StewardCardDragger');
    expect(team).not.toContain('deckShiftFromAxes');
    expect(team).not.toContain('translateX');
    expect(team).not.toContain('pnl');
    expect(team).not.toContain('win_rate');
    expect(team).toContain('pulse');
    expect(team).toContain('heartbeat_at');
    ...
    expect(pager).toContain('canBeginPull');
    expect(pager).toContain('onRefresh');
    expect(pager).toContain('desk-pull');
    expect(pager).toContain('desk-pager-shell');
    expect(app).toContain('PUBLIC_DESK_TABS');
    expect(app).toContain('<DeskPager');
    expect(app).toContain('onRefresh={reloadLedger}');
    expect(app).toContain('theses: (');
    ...
    expect(world).toContain('assembleThesisRoster');
    expect(world).toContain('thesis-list');
    expect(world).toContain('thesis-card');
    expect(world).toContain('thesis-page');
    expect(world).toContain('Back to theses');
    ...
    expect(world).toContain('BeliefQueue');
    expect(world).toContain('TaggedLotQueue');
    expect(world).toContain('LearningPulseMast');
    expect(world).toContain('onOpenThesis={openThesis}');
    expect(world).toContain('HTMLButtonElement');
    expect(world).not.toContain('<p>{formatLearningPulse(pulse)}</p>');
    const mast = await readDashboard('app/terminal/learning-pulse-mast.tsx');
    expect(mast).toContain('assembleLearningPulse');
    expect(mast).toContain('formatLearningPulse');
    expect(mast).toContain('learningPulseParts');
    expect(mast).toContain('assembleBeliefsInForce');
    expect(mast).toContain('learning-pulse');
    expect(mast).toContain('visually-hidden');
    expect(mast).toContain('aria-label="Learning loop"');
    expect(mast).toContain('learning-pulse-chip');
    expect(mast).toContain('data-pulse-beliefs');
    expect(mast).toContain('data-pulse-thesis');
    expect(mast).toContain('data-pulse-lots');
    expect(mast).toContain('data-pulse-lessons');
    expect(mast).toContain('humanizeRule');
    ...
    expect(pulse).toContain('tagged lot');
    expect(pulse).not.toContain('open books tagged');
    expect(pulse).not.toContain("=== 1 ? 'thesis'");
    ...
    expect(css).toContain('.learning-pulse-beliefs');
    expect(css).toContain('.is-pulse-target');
    ...
    expect(beliefs).toContain('Newest playbook rule per thesis.');
    ...
    expect(beliefs).toContain('id="beliefs-in-force"');
    expect(beliefs).toContain('id="tagged-lots"');
    expect(beliefs).toContain('tabIndex={-1}');
    ...
    expect(lessons).toContain('Open first, then in playbook.');
    expect(lessons).not.toContain('${open} open');
    expect(lessons).toContain('id="lessons-parchment"');
    expect(lessons).toContain('tabIndex={-1}');
    ...
    expect(review).toContain('Membership first, ledger score.');
    expect(review).not.toContain('${queue.length} pending');
    ...
    expect(world).not.toContain('thesis-districts');
    ...
    expect(roster).toContain("kind === 'theme'");
    expect(roster).toContain('isLiveThesis');
    expect(roster).not.toContain('win_rate');
    expect(roster).not.toContain('buildings');
    expect(css).toContain('.thesis-world');
    expect(css).toContain('.thesis-list');
    expect(css).toContain('.thesis-card');
    expect(css).not.toContain('.thesis-districts');
    expect(css).not.toContain('scroll-snap-type: y mandatory');
    expect(css).toContain('flex-direction: column');
    expect(world).not.toContain('data-axis="y"');
    expect(css).not.toContain('.thesis-island');
    ...
    expect(css).toContain('.desk-pager[data-axis="y"]');
    expect(css).toContain('.desk-pull');
    ...
    expect(css).toContain('.team-roster');
    expect(css).toContain('.team-card');
    expect(css).not.toContain('.steward-deck');
    expect(css).not.toContain('.steward-dragger');
    expect(css).toMatch(/\.term\.is-swipe \.steward-stage \{[\s\S]*?overflow-y: auto/);
  ...
  test('Board stays Liveline-first; Book is CRT edge, not a Rive tape', async () => {
    ...
    expect(book).not.toContain('crt-book-readout');
    expect(book).not.toContain('StewardEdgeCard');
    ...
    expect(board).toContain('paper-title');
    expect(board).toContain('line-art');
    expect(board).toContain('boardHeroLiveline');
    expect(board).toContain('boardHeroIdle');
    expect(board).toContain('series={hero.series}');
    expect(board).toContain('unit={hero.unit}');
    expect(board).toContain('showValue={hero.showValue}');
    expect(board).toContain('parchment');
    expect(board).toContain('quiet');
    expect(board).toContain('idle={stale}');
    expect(board).toContain('aria-label="Desk return lines"');
    expect(board).toContain('line-key');
    expect(board).not.toContain('bookCurve');
    expect(board).not.toContain('aria-label="Desk NAV"');
    expect(board).not.toContain('Agentic NAV');
    expect(board).not.toContain('lead ${lead.steward}');
    expect(board).not.toContain('ledgerAmount(row.start');
    expect(board).not.toContain('points={nav');
    expect(css).toContain('pointer-events: none');
    expect(css).toContain('.line-frame.is-quiet');
    ...
    expect(css).toContain('.book-life');
    expect(css).toContain('.team-roster');
    expect(css).toContain('.thesis-card');
    ...
    expect(wrap).toContain("theme={parchment ? 'light' : 'dark'}");
    expect(wrap).toContain('LIVELINE_PARCHMENT');
    expect(wrap).toContain('livelineIdle');
    expect(wrap).toContain('lerpSpeed={LIVELINE_LERP_SPEED}');
    expect(wrap).toContain('scrub={!quiet && !compact && !motion.coarse}');
    expect(wrap).toContain('seriesToggleCompact={!quiet && motion.coarse}');
    expect(wrap).toContain('is-hero-value');
    expect(wrap).toContain('fill={multi.length < 2}');
    expect(wrap).toContain('valueMomentumColor={showValue && !freeze}');
    expect(wrap).toContain('referenceLine');
    expect(wrap).toContain('compact');
    expect(wrap).toContain("compact || quiet ? () => ''");
    ...
    expect(openStrip).toContain('StewardAvatar');
    ...
    expect(openStrip).toContain('clip_note.belief');
